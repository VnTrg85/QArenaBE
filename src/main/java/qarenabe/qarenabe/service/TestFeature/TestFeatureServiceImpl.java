...
public class TestFeatureServiceImpl implements TestFeatureService{
    ...
    public List<TestFeatureDTO> getDetailFeaturesByTestProject(Long testProjectId) {
        try {
            ...
            List<TestFeatureDTO> listRes = new ArrayList<>();
            for (TestFeature testFeature : listData) {
                TestFeatureDTO val = new TestFeatureDTO(testFeature.getId(), testFeature.getName(), testFeature.getInput(),testFeature.getOutput(),testFeature.getBugType());
                listRes.add(val);
            }
            return listRes;
        }catch (EntityNotFoundException e) {
    ...
    public TestFeatureDTO createTestFeature(TestFeature testFeature) {
        try {
            ...
            TestFeatureDTO val = new TestFeatureDTO(res.getId(), res.getName(),res.getInput(),res.getOutput(), res.getBugType());
            return val;
        } catch (Exception e) {
